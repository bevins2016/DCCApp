...
import com.S2013.dcc.helpers.BitmapCache;
...
public class NewsArrayAdapter extends ArrayAdapter<News> {
    ...
    private HashMap<String, Boolean> pending = new HashMap<String, Boolean>();
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        View rowView = (view == null) ? inflater.inflate(R.layout.news_item, parent, false) : view;
        assert rowView != null;
    ...
    public class GetImageTask extends AsyncTask<User, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(User... user) {
            Bitmap image;
            try{
                uri = "/DCC/getUserGravitar.php?email=" + user[0].getEmail();

                if(pending.containsKey(uri)){
                    boolean pend = pending.get(uri);
                    while(true){
                        if (!(pending.get(uri))) break;
                        Thread.sleep(10);
                        icon.setImageBitmap(BitmapCache.getBitmap(uri));
                        this.cancel(true);
                        return null;
                    }
                }else{
                    pending.put(uri, true);
                }
